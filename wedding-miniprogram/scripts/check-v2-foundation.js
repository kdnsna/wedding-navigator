...
function checkExperienceContracts() {
  ...
  assert(wizard.includes('visual_preset') && wizard.includes('visualOptions'), 'wizard must persist and preview visual story presets')
  assert(wizard.includes('analyzeWeddingPhoto') && wizard.includes('coverFocus'), 'wizard must inspect photo quality and expose cover focus')
  assert(read('pages-owner/invitation/edit.vue').includes('scenarioPreset'), 'editor must separate scenario preset from theme')
  assert(read('pages-owner/invitation/edit.vue').includes('visualPreset'), 'editor must separate visual preset from scenario and theme')
  const visualPresets = read('utils/visual-presets.js')
  for (const preset of ['cinematic-documentary', 'new-chinese-ceremony', 'garden-film', 'editorial-couture', 'night-banquet']) {
    assert(visualPresets.includes(`id: '${preset}'`), `visual preset is missing: ${preset}`)
  }
  assert(weddingStore.includes('resolveVisualPreset') && weddingStore.includes('getVisualPresetClass'), 'wedding store must normalize and apply visual presets')
  assert(read('pages/index/index.vue').includes('generateWeddingShareCard'), 'guest home must generate a composed WeChat share card')
  assert(read('utils/posterCanvas.js').includes('mountedLayout'), 'poster renderer must vary composition by visual preset')
  assert(read('cloudfunctions/createWedding/index.js').includes('resolveVisualPreset'), 'createWedding must normalize visual presets')
  assert(read('cloudfunctions/updateWedding/index.js').includes('resolveVisualPreset'), 'updateWedding must normalize visual presets')
