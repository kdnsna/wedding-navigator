...
function checkUploadScript() {
  ...
  assert(source.includes("combinedOutput.includes('[error]')"), 'upload script must fail when WeChat CLI prints an error with exit code 0')
  assert(source.includes('fs.statSync(infoOutput).size === 0'), 'upload script must fail when WeChat CLI does not write upload info')
...
function checkFriendsInvitationIteration() {
  ...
  assertIncludes('scripts/smoke-cloud-e2e.mjs', 'smokePosterRuntime', 'CloudBase E2E smoke must verify poster preview runtime layout')
  assertIncludes('scripts/smoke-cloud-e2e.mjs', 'Poster preview image did not render', 'poster runtime smoke must fail if generated image preview is missing')
  ...
  assertIncludes('utils/photoAlbum.js', 'scope.writePhotosAlbum', 'poster saving must request photo album write permission explicitly')
  assertIncludes('utils/photoAlbum.js', 'destWidth: POSTER_CANVAS_WIDTH * POSTER_CANVAS_DPR', 'poster saving must export a full-resolution canvas image')
  assertIncludes('utils/photoAlbum.js', '保存图片或视频到相册', 'poster saving must explain WeChat privacy declaration requirements')
  assertIncludes('utils/photoAlbum.js', 'savePosterFileToAlbum', 'poster saving must support saving the generated preview file without visible canvas')
  assertIncludes('utils/posterCanvas.js', 'fallbackTimer', 'poster drawing must continue when offscreen canvas draw callback is delayed')
  assertIncludes('utils/posterCanvas.js', 'renderWeddingPosterTempFile', 'poster page must export through a Canvas 2D node temp file')
  assertIncludes('pages/poster/index.vue', 'safe-area-inset-top', 'poster page custom header must respect iPhone safe area')
  assertIncludes('pages/poster/index.vue', 'statusBarHeight', 'poster page custom header must use runtime status bar height on iPhone')
  assertIncludes('pages/poster/index.vue', 'type="2d"', 'poster page canvas must use Canvas 2D node export')
  assertIncludes('pages/poster/index.vue', 'posterPreviewPath', 'poster page must show an image preview instead of a visible native canvas')
  assertIncludes('pages/poster/index.vue', 'data-ready', 'poster page save button must expose a stable readiness flag for runtime smoke')
  assertIncludes('pages/poster/index.vue', 'poster-canvas-export', 'poster page canvas must be offscreen to avoid covering buttons on iPhone')
  assertIncludes('pages/poster/index.vue', 'left: -9999px', 'poster page export canvas must be positioned outside the viewport, not merely scaled down')
  assertIncludes('pages/poster/index.vue', 'savePosterFileToAlbum', 'poster page must save the generated poster preview file')
  assertIncludes('pages-owner/poster/index.vue', 'posterPreviewPath', 'owner poster page must show an image preview instead of a visible native canvas')
  assertIncludes('pages-owner/poster/index.vue', 'type="2d"', 'owner poster page canvas must use Canvas 2D node export')
  assertIncludes('pages-owner/poster/index.vue', 'poster-canvas-export', 'owner poster page canvas must be offscreen to avoid covering buttons on iPhone')
  assertIncludes('pages-owner/poster/index.vue', 'left: -9999px', 'owner poster export canvas must be positioned outside the viewport, not merely scaled down')
  assertIncludes('pages-owner/template/preview.vue', 'safe-area-inset-top', 'template custom preview header must respect iPhone safe area')
