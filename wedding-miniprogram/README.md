...
相册上传稳定性修复：主人端相册页会先触发微信隐私授权，再按 `wx.chooseImage`、`uni.chooseImage`、`wx.chooseMedia` 顺序从手机相册选择照片，并在上传前同时确保 `wx.cloud` 与 `uni.cloud` 已初始化，避免部分机型出现“选择照片失败”或“选图成功但云存储上传失败”。微信公众平台隐私保护指引需声明“照片或视频信息”用途。
