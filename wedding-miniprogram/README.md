...
本轮继续把路书从“通用信息页”升级为可信的宾客到场助手：

- 天气改用 7 日预报并按婚礼日期精确匹配，不再把接口第一天的天气误标成婚礼当天
- 婚期超过预报窗口、日期无效、缺坐标或天气服务不可用时，只展示准确的待更新状态，不再生成模拟晴天和温度
- 已提交回执的宾客会在路书看到自己的赴约状态、人数、预计到达时间和交通方式，并可直接修改回执
- `npm run check:release` 新增天气日期边界、精确日匹配和“禁止伪造天气”契约检查
...
相册上传稳定性修复：主人端相册页会在用户点击上传时触发微信官方隐私授权，再按 `wx.chooseImage`、`uni.chooseImage`、`wx.chooseMedia` 顺序从手机相册选择照片，并在上传前同时确保 `wx.cloud` 与 `uni.cloud` 已初始化。微信公众平台需在“设置 → 服务内容声明 → 用户隐私保护指引”中分别声明：“收集你选中的照片或视频信息”，用途填写“用于新人上传婚礼照片并制作婚礼相册、请柬封面与分享海报”；“使用你的相册（仅写入）”，用途填写“用于您主动点击‘保存到相册’时，将小程序生成的婚礼分享海报写入手机相册，便于分享邀请”。保存后约 5 分钟生效。
...
- 路书天气页对宾客只展示准确预报或友好的待更新状态，云端仍保留缺 Key、缺坐标和服务异常原因用于排查
...
- `cloudbaserc.json` 已补齐全部 18 个云函数，避免上线时漏部署宾客轻量读取、祝福分页、创建、更新、RSVP、统计、AI 发布助手等核心函数
...
### 主人端（编辑管理）
- **创建向导**：四幕流程创建婚礼（具名→择地→选照→定色成书），照片会即时装裱预览，场景方案负责预填文案、流程和路书
...
未配置时只返回明确的“天气待更新”状态，不伪造温度和晴雨信息，也不会阻断路书页。
...
`cloudbaserc.json` 已包含全部 18 个云函数。如果分享设置页提示小程序码生成失败，请优先确认 `generatePoster` 已部署，并已在云函数权限中开启 `wxacode.getUnlimited`。如果主人端 AI 发布助手提示暂不可用，请确认 `aiPublishAssistant` 已部署、已云端安装依赖，并已启用 CloudBase AI 模型服务。
...
- 已按 `cloudbaserc.json` 部署全部 18 个 `cloudfunctions/*`，选择「云端安装依赖」
- 已确认 `dist/build/mp-weixin/cloudfunctions` 存在 18 个云函数目录；如不存在，请重新运行 `npm run build:mp-weixin`
...
- 已在微信公众平台“设置 → 服务内容声明 → 用户隐私保护指引”声明“收集你选中的照片或视频信息”和“使用你的相册（仅写入）”，保存约 5 分钟后重新真机验证相册上传与海报保存
...
- 7 张场景方案原图保存在仓库 `assets/scenario-heroes/`；生产发布前需上传云存储并回填 `TEMPLATE_HERO_IMAGES`，主包只保留纸面兜底图
...
场景方案原图集中在仓库 `assets/scenario-heroes/`。生产环境上传云存储并回填 `TEMPLATE_HERO_IMAGES` 后，才会按当前场景加载；未回填时明确回落到主包内唯一的 `default-cover.png`。首页、完整方案预览和 Canvas 分享海报始终优先使用新人自己上传的相册封面。
...
| 权限 | 读写全部 | 只读 + RSVP/祝福 |
| 页面 | 我的书案（含发布诊断、账号权益） | 动态文本导航（请柬/赴约/路书/更多，按数据隐藏） |
