...
2026-07-16 隐私审核整改：已将“选中婚礼照片并上传”与“用户主动保存婚礼分享海报到相册”拆为读取/上传和相册仅写入两个独立场景；小程序隐私页、发布诊断和发布检查已统一口径。
...
| 项目 | 修复内容 | 上线影响 |
|------|----------|----------|
| 云函数部署清单 | `cloudbaserc.json` 从 5 个函数补齐为 16 个函数，并统一 runtime、handler、timeout、description | 避免只部署地图/天气/海报/AI 助手等部分函数，导致创建、更新、RSVP、祝福、统计等链路线上缺函数 |
...
| 兜底数据 | 创建向导日期与时间初始为空，宾客无有效邀请时不渲染虚构姓名、婚期或回执 | 避免测试默认值被误保存或伪装成真实婚书 |
...
| `getWeather` | 坐标缺失时尝试地理编码，缺 Key、超出预报窗口或服务异常时返回明确待更新状态 | 依赖地图 Key + `HEFENG_KEY` / `QWEATHER_KEY` / `WEATHER_KEY`，禁止伪造天气 | 需确认环境变量 |
...
8. 在微信公众平台声明“收集你选中的照片或视频信息”，保存约 5 分钟后，用微信开发者工具打开 `dist/build/mp-weixin`，完整跑通：创建婚礼 -> 选模板 -> 编辑婚书 -> 上传照片 -> 配路书坐标 -> 提交 RSVP -> 写祝福 -> 生成小程序码/海报 -> 分享进入 -> 删除婚礼后旧链接失效。
9. 确认 `aiPublishAssistant` 已安装 `@cloudbase/node-sdk` 依赖，并已启用 CloudBase AI 模型服务；未启用时主人端应显示“AI 暂不可用”，手动编辑不受影响。
10. 在真机上检查 375px、390px、430px 常见宽度：无文字重叠、无按钮截断、无底部按钮遮挡、模板预览图片主体不被顶部裁掉。
