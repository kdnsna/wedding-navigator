# 甜囍手册

> 主人 2026年11月14日婚礼的专属婚礼导航小程序

## 项目简介

「甜囍手册」是一款专为婚礼打造的微信小程序，集**婚书请柬**、**婚纱相册**、**婚礼路书**、**流程时间线**、**RSVP宾客管理**、**祝福墙**于一体，建立「主人编辑端 + 宾客展示端」双角色体系。

### 核心功能

- **婚书请柬**：3种精美模板风格，支持背景音乐设置
- **婚纱相册**：上传/瀑布流/轮播展示
- **婚礼路书**：地图标记、一键导航、天气预报、交通住宿指引
- **流程时间线**：婚礼当天完整时间轴，角色筛选，当前节点自动高亮
- **RSVP宾客管理**：回执收集、出席统计、饮食偏好记录
- **祝福墙**：文字祝福提交、主人审核置顶
- **海报生成**：Canvas 绘制精美分享海报，含小程序码

## 技术栈

- **框架**：uni-app (Vue3) + Vite
- **UI**：uni-ui + 自定义婚礼主题组件
- **状态管理**：Pinia
- **后端**：微信云开发（CloudBase）— 数据库 + 存储 + 云函数
- **地图**：腾讯位置服务

## 快速开始

### 1. 克隆项目

```bash
git clone https://github.com/sanchui/wedding-navigator.git
cd wedding-navigator/wedding-miniprogram
```

### 2. 安装依赖

```bash
pnpm install
```

### 3. 配置云开发环境

修改 `config/cloud.js` 中的 `CLOUD_ENV` 为你的云开发环境 ID：

```js
export const CLOUD_ENV = 'your-cloud-env-id'
```

### 4. 运行到微信开发者工具

```bash
pnpm dev:mp-weixin
```

### 5. 部署云函数

在微信开发者工具中，右键 `cloudfunctions/` 下的每个云函数目录，选择「上传并部署：云端安装依赖」。

## 项目结构

```
wedding-miniprogram/
├── pages/              # 宾客端（7页）
├── pages-owner/        # 主人端（10页，subPackages分包）
├── cloudfunctions/     # 云函数（11个）
├── components/         # 公共组件
├── stores/             # Pinia 状态管理
├── composables/        # 组合式逻辑
├── utils/              # 工具函数
├── config/             # 云配置
└── uni.scss            # 全局设计系统
```

## 设计文档

- `DESIGN-v2.md` — 综合设计方案
- `DESIGN.md` — 参考方案1
- `assets/小程序设计方案.md` — 参考方案2

## 开发记录

| 阶段 | 内容 | 状态 |
|------|------|------|
| Phase 1 | 婚书 + 相册 + 路书 + 分享 | ✅ |
| Phase 2 | 流程 + RSVP + 祝福墙 | ✅ |
| Phase 3 | 体验优化 + 数据统计 | ✅ |
| Phase 4 | UI 视觉全面升级（中国红+香槟金） | ✅ |
| Phase 5 | 高级简约无界设计重构 | ✅ |
| Phase 6 | 海报生成 + 天气 + 交通住宿 + 背景音乐 | ✅ |
